...
squaredDifferences = zeros(size(K));
recognitionRates = zeros(size(K));

% calculating the eigenvectors for each k
for i = 1:size(K, 2)
    eV = getEigenvectors(trainingSet, K(i));
...
title('Recognition rate vs K plot');

%% Yale Database testing
